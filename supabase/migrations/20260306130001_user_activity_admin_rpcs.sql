-- 5) Admin moderation RPC: richer activity fields in user list.
...
-- 6) Admin user detail RPC: include richer activity diagnostics.
...
last_content AS (
    SELECT MAX(x.created_at) AS last_content_at
    FROM (
        ...
        FROM public.reviews rv
        WHERE rv.user_id = target_user_id
        UNION ALL
        SELECT s.created_at
        FROM public.suggested_fuel_stations s
        WHERE s.submitted_by = target_user_id
        UNION ALL
        SELECT fs.created_at
        FROM public.flagged_stations fs
        WHERE fs.user_id = target_user_id
        UNION ALL
        SELECT fav.created_at
        FROM public.favourite_stations fav
...
SELECT
    ...
    CASE
        WHEN derived.last_activity_at IS NULL THEN 'inactive'
        WHEN derived.last_activity_at >= now() - interval '15 minutes' THEN 'online'
        WHEN derived.last_activity_at >= now() - interval '24 hours' THEN 'today'
        WHEN derived.last_activity_at >= now() - interval '7 days' THEN 'this_week'
        ELSE 'inactive'
    END AS activity_status,
    e24.events_24h,
    r7.reports_7d,
    ...
    fav7.favourites_7d,
    (
        COALESCE(r7.reports_7d, 0)
        + COALESCE(rv7.reviews_7d, 0)
        + COALESCE(s7.suggestions_7d, 0)
        + COALESCE(f7.flags_7d, 0)
        + COALESCE(fav7.favourites_7d, 0)
        + COALESCE(e24.events_24h, 0)
    )::bigint AS activity_score_7d,
    te.total_activity_events
FROM user_base ub
LEFT JOIN last_seen ls ON true
LEFT JOIN event_24h e24 ON true
LEFT JOIN total_events te ON true
LEFT JOIN reports_7d r7 ON true
LEFT JOIN reviews_7d rv7 ON true
LEFT JOIN suggestions_7d s7 ON true
LEFT JOIN flags_7d f7 ON true
LEFT JOIN favourites_7d fav7 ON true
LEFT JOIN public.user_activity_summary uas ON uas.user_id = ub.id
LEFT JOIN last_content lc ON true
LEFT JOIN LATERAL (
    SELECT NULLIF(
        GREATEST(
            COALESCE(ub.last_sign_in_at, '-infinity'::timestamptz),
            COALESCE(uas.last_active_at, '-infinity'::timestamptz),
            COALESCE(ls.last_seen_at, '-infinity'::timestamptz),
            COALESCE(lc.last_content_at, '-infinity'::timestamptz)
        ),
        '-infinity'::timestamptz
    ) AS last_activity_at
) derived ON true;
...
-- 7) Stats RPC for moderation dashboard cards.
DROP FUNCTION IF EXISTS public.get_user_activity_stats();
...
user_activity AS (
    ...
    FROM public.profiles p
    LEFT JOIN auth.users au ON au.id = p.id
    LEFT JOIN public.user_activity_summary uas ON uas.user_id = p.id
    LEFT JOIN device_last_seen dls ON dls.user_id = p.id
    LEFT JOIN content_last_activity cla ON cla.user_id = p.id
),
contributors AS (
    SELECT COUNT(DISTINCT x.user_id)::bigint AS contributors_7d
    FROM (
        SELECT pr.user_id
        FROM public.price_reports pr
        WHERE pr.created_at >= now() - interval '7 days'
...
SELECT
    ...
    COUNT(*) FILTER (WHERE ua.last_activity_at >= date_trunc('day', now()))::bigint AS active_today,
    COUNT(*) FILTER (WHERE ua.last_activity_at >= now() - interval '7 days')::bigint AS active_7d,
    COUNT(*) FILTER (WHERE ua.created_at >= now() - interval '7 days')::bigint AS new_this_week,
    MAX(c.contributors_7d)::bigint AS contributors_7d,
    COUNT(*) FILTER (WHERE ua.last_activity_at >= now() - interval '15 minutes')::bigint AS online_now
